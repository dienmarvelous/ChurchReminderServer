...
app.post('/send-call', async (req, res) => {
  ...
  try {
    ...
    switch ((provider || 'africastalking').toLowerCase()) {
      ...
      // ── Twilio ────────────────────────────────────
      case 'twilio': {
        // Build TwiML to speak church reminder message when call connects
        const callMessage = req.body.message || 'Hello, this is a reminder from your church. Sunday Service is tomorrow morning. We look forward to worshipping with you. God bless you.';
        const twiml = `<?xml version="1.0" encoding="UTF-8"?><Response><Say voice="alice" language="en-GB">${callMessage}</Say><Pause length="1"/><Say voice="alice" language="en-GB">God bless you.</Say></Response>`;

        const twilioBody = new URLSearchParams();
        twilioBody.append('To', to);
        twilioBody.append('From', from);
        twilioBody.append('Twiml', twiml);
